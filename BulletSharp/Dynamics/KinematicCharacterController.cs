...
namespace BulletSharp
{
    public class KinematicCharacterController : ICharacterController
    {
        ...
        protected bool RecoverFromPenetration(CollisionWorld collisionWorld)
        {
            ...
            for (int i = 0; i < m_ghostObject.OverlappingPairCache.NumOverlappingPairs; i++)
            {
                ...
                for (int j = 0; j < m_manifoldArray.Count; j++)
                {
                    ...
                    Scalar directionSign = manifold.Body0 == m_ghostObject ? -1f : 1f;
                    for (int p = 0; p < manifold.NumContacts; p++)
                    {
                        ManifoldPoint pt = manifold.GetContactPoint(p);

                        Scalar dist = pt.Distance;
        ...
        protected void UpdateTargetPositionBasedOnCollision(ref Vector3 hitNormal, Scalar tangentMag, Scalar normalMag)
        {
        ...
        protected void StepForwardAndStrafe(CollisionWorld collisionWorld, ref Vector3 walkMove)
        {
            ...
            Scalar fraction = 1.0f;
            Scalar distance2 = (m_currentPosition - m_targetPosition).LengthSquared;
            //	printf("distance2=%f\n",distance2);
            ...
            while (fraction > 0.01f && maxIter-- > 0)
            {
                ...
                Scalar margin = m_convexShape.Margin;
                ...
                if (callback.HasHit)
                {
                    ...
                    Scalar hitDistance = (callback.HitPointWorld - m_currentPosition).Length;
        ...
        protected void StepDown(CollisionWorld collisionWorld, Scalar dt)
        {
            ...
            Scalar downVelocity = (m_verticalVelocity < 0.0f ? -m_verticalVelocity : 0.0f) * dt;
            ...
            while (true)
            {
                ...
                Scalar downVelocity2 = (m_verticalVelocity < 0.0f ? -m_verticalVelocity : 0.0f) * dt;
                bool has_hit = false;
                if (bounce_fix == true)
            ...
            if (callback.HasHit || runonce == true)
            {
                ...
                Scalar fraction = (m_currentPosition.Y - callback.HitPointWorld.Y) / 2;
        ...
        public KinematicCharacterController(PairCachingGhostObject ghostObject, ConvexShape convexShape, Scalar stepHeight, int upAxis = 1)
        {
            m_upAxis = upAxis;
            m_addedMargin = 0.02f;
        ...
        public virtual void UpdateAction(CollisionWorld collisionWorld, Scalar deltaTime)
        {
        ...
        public void SetVelocityForTimeInterval(ref Vector3 velocity, Scalar timeInterval)
        {
            //	printf("setVelocity!\n");
            //	printf("  interval: %f\n", timeInterval);
        ...
        public void SetVelocityForTimeInterval(Vector3 velocity, Scalar timeInterval)
        {
        ...
        public void PlayerStep(CollisionWorld collisionWorld, Scalar dt)
        {
            // quick check...
        ...
        public void SetFallSpeed(Scalar fallSpeed)
        {
            m_fallSpeed = fallSpeed;
        }

        public void SetJumpSpeed(Scalar jumpSpeed)
        {
            m_jumpSpeed = jumpSpeed;
        }

        public void SetMaxJumpHeight(Scalar maxJumpHeight)
        {
        ...
        public void Jump()
        {
            if (CanJump)
            {
                ...
                //Vector3 up = xform.Up;
                //up.Normalize ();
                //Scalar magnitude = (1.0f/m_rigidBody.getInvMass()) * 8.0f;
        ...
        public Scalar Gravity { get; set; }

        /// The max slope determines the maximum angle that the controller can walk up.
        /// The slope angle is measured in radians.
        public Scalar MaxSlope
        {
            get { return m_maxSlopeRadians; }
            set
            {
                m_maxSlopeRadians = value;
                m_maxSlopeCosine = (Scalar)System.Math.Cos(value);
            }
        }
        ...
        protected Scalar m_verticalVelocity;
        protected Scalar m_verticalOffset;


        protected Scalar m_fallSpeed;
        protected Scalar m_jumpSpeed;
        protected Scalar m_maxJumpHeight;
        protected Scalar m_maxSlopeRadians; // Slope angle that is set (used for returning the exact value)
        protected Scalar m_maxSlopeCosine;  // Cosine equivalent of m_maxSlopeRadians (calculated once when set, for optimization)

        protected Scalar m_turnAngle;

        protected Scalar m_stepHeight;

        protected Scalar m_addedMargin;//@todo: remove this and fix the code
        ...
        protected Scalar m_velocityTimeInterval;
    ...
    public class KinematicClosestNotMeRayResultCallback : ClosestRayResultCallback
    {
        ...
        public override Scalar AddSingleResult(LocalRayResult rayResult, bool normalInWorldSpace)
        {
    ...
    public class KinematicClosestNotMeConvexResultCallback : ClosestConvexResultCallback
    {
        ...
        public KinematicClosestNotMeConvexResultCallback(CollisionObject me, Vector3 up, Scalar minSlopeDot)
            : base(ref zero, ref zero)
        {
            _me = me;
            _up = up;
        ...
        public override Scalar AddSingleResult(LocalConvexResult convexResult, bool normalInWorldSpace)
        {
