...
namespace BulletSharp
{
	public abstract class DebugDraw : IDisposable
	{
		...
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawArcUnmanagedDelegate([In] ref Vector3 center, [In] ref Vector3 normal, [In] ref Vector3 axis, Scalar radiusA, Scalar radiusB,
			Scalar minAngle, Scalar maxAngle, ref Vector3 color, bool drawSect, Scalar stepDegrees);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		...
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawCapsuleUnmanagedDelegate(Scalar radius, Scalar halfHeight, int upAxis, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawConeUnmanagedDelegate(Scalar radius, Scalar height, int upAxis, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawContactPointUnmanagedDelegate([In] ref Vector3 pointOnB, [In] ref Vector3 normalOnB, Scalar distance, int lifeTime, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawCylinderUnmanagedDelegate(Scalar radius, Scalar halfHeight, int upAxis, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawLineUnmanagedDelegate([In] ref Vector3 from, [In] ref Vector3 to, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawPlaneUnmanagedDelegate([In] ref Vector3 planeNormal, Scalar planeConst, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawSphereUnmanagedDelegate(Scalar radius, [In] ref Matrix transform, [In] ref Vector3 color);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawSpherePatchUnmanagedDelegate([In] ref Vector3 center, [In] ref Vector3 up, [In] ref Vector3 axis, Scalar radius,
			Scalar minTh, Scalar maxTh, Scalar minPs, Scalar maxPs, [In] ref Vector3 color, Scalar stepDegrees);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawTransformUnmanagedDelegate([In] ref Matrix transform, Scalar orthoLen);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		delegate void DrawTriangleUnmanagedDelegate([In] ref Vector3 v0, [In] ref Vector3 v1, [In] ref Vector3 v2, [In] ref Vector3 color, Scalar alpha);
		[UnmanagedFunctionPointer(Native.Conv), SuppressUnmanagedCodeSecurity]
		...
		public virtual void DrawArc(ref Vector3 center, ref Vector3 normal, ref Vector3 axis, Scalar radiusA, Scalar radiusB,
			Scalar minAngle, Scalar maxAngle, ref Vector3 color, bool drawSect, Scalar stepDegrees = 10.0f)
		{
			Vector3 xAxis = radiusA * axis;
			Vector3 yAxis = radiusB * Vector3.Cross(normal, axis);
			Scalar step = stepDegrees * MathUtil.SIMD_RADS_PER_DEG;
			int nSteps = (int)((maxAngle - minAngle) / step);
			if (nSteps == 0)
			{
				nSteps = 1;
			}
			Vector3 prev = center + xAxis * (Scalar)System.Math.Cos(minAngle) + yAxis * (Scalar)System.Math.Sin(minAngle);
			if (drawSect)
			{
			...
			{
				Scalar angle = minAngle + (maxAngle - minAngle) * i / nSteps;
				Vector3 next = center + xAxis * (Scalar)System.Math.Cos(angle) + yAxis * (Scalar)System.Math.Sin(angle);
		...
		public virtual void DrawCapsule(Scalar radius, Scalar halfHeight, int upAxis, ref Matrix transform, ref Vector3 color)
		{
		...
		public virtual void DrawCone(Scalar radius, Scalar height, int upAxis, ref Matrix transform, ref Vector3 color)
		{
			Vector3 start = transform.Origin;
		...
		public virtual void DrawContactPoint(ref Vector3 pointOnB, ref Vector3 normalOnB, Scalar distance, int lifeTime, ref Vector3 color)
		{
			Vector3 to = pointOnB + normalOnB * 1; // distance
			DrawLine(ref pointOnB, ref to, ref color);
		}

		public virtual void DrawCylinder(Scalar radius, Scalar halfHeight, int upAxis, ref Matrix transform, ref Vector3 color)
		{
			Vector3 start = transform.Origin;
			Matrix basis = transform.Basis;
			Vector3 offsetHeight = Vector3.Zero;
		...
		public virtual void DrawPlane(ref Vector3 planeNormal, Scalar planeConst, ref Matrix transform, ref Vector3 color)
		{
			Vector3 planeOrigin = planeNormal * planeConst;
			Vector3 vec0, vec1;
			PlaneSpace1(ref planeNormal, out vec0, out vec1);
			const Scalar vecLen = 100f;
		...
		public virtual void DrawSphere(Scalar radius, ref Matrix transform, ref Vector3 color)
		{
			Vector3 start = transform.Origin;
			Matrix basis = transform.Basis;
		...
		public virtual void DrawSphere(ref Vector3 p, Scalar radius, ref Vector3 color)
		{
			Matrix tr = Matrix.Translation(p);
			DrawSphere(radius, ref tr, ref color);
		}

		public virtual void DrawSpherePatch(ref Vector3 center, ref Vector3 up, ref Vector3 axis, Scalar radius,
			Scalar minTh, Scalar maxTh, Scalar minPs, Scalar maxPs, ref Vector3 color)
		{
			DrawSpherePatch(ref center, ref up, ref axis, radius, minTh, maxTh, minPs, maxPs, ref color, 10.0f);
		}

		public virtual void DrawSpherePatch(ref Vector3 center, ref Vector3 up, ref Vector3 axis, Scalar radius,
			Scalar minTh, Scalar maxTh, Scalar minPs, Scalar maxPs, ref Vector3 color, Scalar stepDegrees)
		{
			Vector3[] vA;
			Vector3[] vB;
			Vector3[] pvA, pvB, pT;
			...
			if (n_hor < 2) n_hor = 2;
			Scalar step_h = (maxTh - minTh) / (n_hor - 1);
			bool isClosed;
			if (minPs > maxPs)
			{
			...
			{
				Scalar th = minTh + i * step_h;
				Scalar sth = radius * (Scalar)System.Math.Sin(th);
				Scalar cth = radius * (Scalar)System.Math.Cos(th);
				for (int j = 0; j < n_vert; j++)
				{
					Scalar psi = minPs + (Scalar)j * step_v;
					Scalar sps = (Scalar)System.Math.Sin(psi);
					Scalar cps = (Scalar)System.Math.Cos(psi);
		...
		public virtual void DrawTriangle(ref Vector3 v0, ref Vector3 v1, ref Vector3 v2, ref Vector3 n0, ref Vector3 n1, ref Vector3 n2, ref Vector3 color, Scalar alpha)
		{
			DrawTriangle(ref v0, ref v1, ref v2, ref color, alpha);
		}

		public virtual void DrawTriangle(ref Vector3 v0, ref Vector3 v1, ref Vector3 v2, ref Vector3 color, Scalar alpha)
		{
		...
		public virtual void DrawTransform(ref Matrix transform, Scalar orthoLen)
		{
			Vector3 start = transform.Origin;
			Matrix basis = transform.Basis;
		...
		public static void PlaneSpace1(ref Vector3 n, out Vector3 p, out Vector3 q)
		{
			if (System.Math.Abs(n.Z) > MathUtil.SIMDSQRT12)
			{
				...
				Scalar a = n.Y * n.Y + n.Z * n.Z;
				Scalar k = MathUtil.RecipSqrt(a);
